...
const EventsEdit = () => {
    ...
    var tagsAll = ['เชียงใหม่' , 'แอดเวนเจอร์' , 'ร้านอาหารแนะนำ' ,'อันซีนไทยแลนด์' , 'อุทยานแห่งชาติ'];
    ...
    useEffect(() => {
        ...
        const reqByIDevent = async () => {
            ...
            try {
                ...
                const response = await endpoint.get("/eventsact/once/"+paramID);
                if(response.data.code === 1){

                    console.log(response.data.list.eventsCate)

                    setCateEvent(response.data.list.eventsCate)
                    setTitleEvent(response.data.list.eventsTitle);
                    setDetailEvent(JSON.parse(response.data.list.eventsDes))

                    if(response.data.list.eventsImglist.length > 0){

                        response.data.list.eventsImglist.forEach((lis) => {

                            imgall.push({filebase64:"" , fileimg:lis})
                        })
                    ...
                    response.data.list.eventsTag.forEach((col , keys) => {

                        tagall.push({id:keys+1 , label:col})
   ...
   const submitEvent = async (e) => {
    ...
    var imgOld = [];
     ...
     if(cateEvent && titleEvent && fileTemp.length > 0 && detailEvent.length > 0 && chipData.length > 0){
         ...
         try {
             const response = await endpoint.patch("/eventsact/once/"+paramID  , frmData, {delay: 500 , headers: {
             ...
             if(response.data.code === 1){
                 ...
                 setSwalProps({...swalProps , show:true  , icon:"success" , title:"แก้ไขข้อมูลเรียบร้อยแล้ว" , showConfirmButton:false})
                ...
                <Grid item xs={12} sx={{p:2}}>
                        <FormControl variant="filled" fullWidth>
                                ...
                                <Select
                                    ...
                                    <MenuItem value='TRAVEL'>สถานที่ท่องเที่ยว</MenuItem>
